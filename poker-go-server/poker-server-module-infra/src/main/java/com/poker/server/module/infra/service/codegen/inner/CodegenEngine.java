package com.poker.server.module.infra.service.codegen.inner;
...
import com.poker.server.framework.apilog.core.annotation.ApiAccessLog;
import com.poker.server.framework.apilog.core.enums.OperateTypeEnum;
...
import com.poker.server.framework.common.util.date.LocalDateTimeUtils;
import com.poker.server.framework.common.util.object.BeanUtils;
import com.poker.server.framework.common.util.object.ObjectUtils;
import com.poker.server.framework.common.util.string.StrUtils;
import com.poker.server.framework.excel.core.annotations.DictFormat;
import com.poker.server.framework.excel.core.convert.DictConvert;
import com.poker.server.framework.excel.core.util.ExcelUtils;
...
import com.poker.server.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.poker.server.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.poker.server.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.poker.server.module.infra.enums.codegen.CodegenSceneEnum;
import com.poker.server.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.poker.server.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.poker.server.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 pokerServer-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 pokerServer-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.poker.server.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/pokerServer-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/pokerServer-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "pokerServer-module-${table.moduleName}/" + // 顶级模块
                "pokerServer-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "pokerServer-module-${table.moduleName}/" + // 顶级模块
                "pokerServer-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "pokerServer-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "pokerServer-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "pokerServer-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "pokerServer-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
