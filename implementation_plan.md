...
### 2.4 经营者管理后台开发（Week 4-6，与后端并行）

| 周次 | 页面 | 关键技术点 |
|------|------|-----------|
| Week 4 | 项目初始化 + 暗色主题 + 登录 + 工作台 | Vite 6 + Vant 4 + UnoCSS、Sa-Token 认证、数据卡片 |
| Week 5 | 活动管理（列表/创建/编辑/排名录入）+ 订单管理 | WebSocket 实时订单通知、滑动操作 |
| Week 6 | 开店向导 + 经营设置 + 商品管理 + 品牌主题 + 会员列表 | 步骤向导组件、主题预览、拖拽排序 |

### 2.5 联调与集成（Week 7）

| 任务 | 说明 |
|------|------|
| 前后端联调 | 小程序 + 管理后台 + 后端 API 全流程跑通 |
| 微信登录联调 | 真机测试微信 OAuth 登录流程 |
| 主题联调 | 验证 6 套主题在小程序中的显示效果 |
| 通知联调 | WebSocket 订单提醒 + 微信模板消息 |
| 数据一致性验证 | 积分计算、订单金额、排行榜排序 |
| 系统后台验证 | 知识库内容管理 + 合规文档 + 主题模板管理 |
...
| 管理后台部署 | 微信云托管（静态托管） | Vite build → 上传 |
| 系统后台部署 | 同一云托管实例 | Nginx 路由分发 |
...
| 后端开发 | 1-2人 | Spring Boot + 数据库 + API |
| 前端开发 | 1-2人 | uni-app 小程序 + Vue 3 管理后台 |
